...
\newcommand\exampleRandomCharacter{
  \renewcommand\csComments{
      ...
      \commentary{[xshift=0em,yshift=-2em]TCBPOSTER@attributes.north}{0em,0em}{{\LARGE \arabic{track}: }Roll to determine each of your \glsfmtplural{attribute} \vpageref{randomCharacterCreation}.}
      \commentary{[xshift=-11em,yshift=-4em]TCBPOSTER@title.north}{-2em,-0.5em}{{\LARGE \arabic{track}: }Roll a random race (\vpageref{raceRoll}), then apply \glsentrytext{attribute} modifiers and roll a name (\vpageref{randomNames}).}
