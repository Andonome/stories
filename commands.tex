...
\newcommand\namesfordwarves{
  ...
  \or
    \dicef{4}   & Ravine        & Mamelon         & Veyor       \\
  \else
    \dicef{4}   & Niche         & Range           & Wordsmith      \\
  \fi
  \ifodd\value{r4}
    \dicef{5}   & Graben        & Morgaine        & Chef        \\
    \dicef{6}   & Riffle        & Scree           & Dener     \\
  \else
    \dicef{5}   & Karst         & Horst           & Brewer      \\
    \dicef{6}   & Scarp         & Glacis          & Chitect   \\
  \fi
  \end{boxtable}
}
  ...
  \begin{tikzpicture}[very thick]
    ...
    \foreach \x in {1,...,5} {
      ...
      \ifnum\value{enc}>5
        \addtocounter{enc}{-5}
      \fi
      \draw[fill=background] (\x) -- (\arabic{enc});
    }

    \foreach \x in {1,...,5} {
      \draw[fill=background] (\x) circle(\sphereBack){};
      \addtocounter{track}{72}
    }
