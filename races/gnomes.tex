...
Their network of tunnels and homes extend often up to fifty \glspl{step} below the ground.
...
\subsubsection[The Gnomish Language]{The Language}
...
Despite the difficulties creating compounds, people learn the basics fast, so Gnomish has formed the basis of \gls{tradeTongue}, which almost everyone uses while travelling.
Unlike \gls{tradeTongue}, Gnomish has three versions --- in addition to being able to speak and write, they can also whistle their language.
...
Many Gnomish societies have complicated electoral systems where members cast differing numbers of votes in order to elect each other into positions of government.
...
Their \glspl{talisman} work similarly -- gnomes often write some activation word on their alchemical creations in the form of a riddle; this ensures that stupid people who don't speak their language cannot use the item, which functions to stop `bad people' using the item.%
\footnote{We all have our little prejudices, and the gnomish intellect does not make them an exception.}

Every trap worth using comes with a Gnomish trademark, as a guarantee of its excellent quality.
...
sit outside Gnomish warrens, as if to say `gee, I sure hope nobody goes into my treasure chest and ambushes me later'.
They have no bait, just a latch which swings down and locks once closed.
Small pieces of wood keep the lid propped open to make it more inviting.
...
Trappers must empty the trap quickly, because if another \gls{woodspy} arrives then it will understand the trap, and wait beside it, in the hopes of catching a gnome.

\subsection{Commerce}
\index{Gnomes!Currency}
\index{Maps|see {Gnomes}: {Currency}\space}

Gnomes use treasure maps as currency, as none of them want to walk long distances with bags of heavy coins.
\ifodd\day%
  Maps allow you to show proof of ownership without going to the effort of carting heavy stuff about.
  This brings the temptation to copy a map and sell it multiple times, which often happens, although gnomes consider it bad form to sell the same treasure more than nine times.
  It also brings the danger of someone (usually a human) tracking down the treasure and actually taking it.

  For extra security, many will store their maps in a secure location and create a map to that location as proof of ownership (what gnome doesn't love recursion?).
  Recently, rumours have begun to circulate of expert thieves stealing all the original gnomish treasure, but since the system works so well it hardly matters.
\else%
...
can be harvested from gnomes' bones with enough grinding (about 2~\glspl{ingredient} per gnome).
