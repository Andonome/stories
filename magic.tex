...
The actual \gls{casting} could express the spell as chatter, humming, sign-language, theatre or song; they all look different, but all express that same \gls{invocation}.
...
\textit{Detailed} Fate spells can inflict Penalties on \glspl{action}, so a target may receive a -3~Penalty the next time they swing a sword (\roll{Dexterity}{Melee}) or the next time \gls{navigation} becomes tricky.
...
Warping \glspl{spell} may also make someone's \glspl{fp} feel unfamiliar, so other spells cannot detect those \glspl{fp} so easily (but they still operate the same).
...
Even small spells can shove people with a small blast of wind, removing \glspl{ap}.
...
\textit{Duplicated Witness} \glspl{spell} let \glspl{witch} map out a space in the dark by asking about many points in space, while \textit{Detailed Witness} spells let them detect specific materials, like `gold' or `leather'.
