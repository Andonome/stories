...
\noindent
`\Glspl{ingredient}' let spell-casters and alchemists alike infuse mana into items.
The items cure diseases, boost spells, and even lock a spell into an item, so that anyone can use it.

\Glspl{ingredient} include griffin feathers, auroch hooves, and marching mushrooms.
Nobody acquires them easily.

Preparation needs a clear kitchen, and clear mind, but never a second set of hands to manage.
Characters cannot use \glspl{restingaction} or Banding actions.

A good kitchen, or even full laboratory, helps a lot.
...
Success means that the \gls{boon} can boost anyone's rating in the appropriate elemental Sphere by 1.
A tie indicates that the \gls{boon} emerges useable, but rank.
It will go rotten within $1D6$ days.

\begin{itemize}
  \item
  \Glspl{boon} demand at least 1 \gls{ap} to extract from a pocket, and 1 expose (by uncorking or unwrapping).
  \item
  \Glspl{boon} remain suffused in the air until someone (anyone) consumes the energy by casting a spell, or until the wind blows the energy away.
  \item
\end{itemize}
...
\paragraph{Failure}
to make \pgls{boon} properly means the energy releases instantly, resulting in a spell from that Sphere.
The type of spell depends on the Failure Margin.

\label{randomSpellFailure}
\begin{enumerate}
  \item
  Warps the element; perhaps nearby fires glow blue.
  \item
  Wanes the element; water evaporates, or luck turns bad.
  \item
  Waxes the element; fire-place ash turns solid, or air blows all the windows open.
\end{enumerate}
...
\begin{enumerate}
  \item
  The alchemist gathers a number of \glspl{ingredient} equal to a spell's Sphere requirement.
...
\paragraph{Failure}
means the spell's energies recombine in the wrong way, producing some random effect, just like \glspl{boon} (\vpageref{randomSpellFailure}).
The \gls{gm} selects the Descriptors, and selects something completely unintended by the original spell.
...
The system works as above, with another step:
