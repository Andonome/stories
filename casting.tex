...
\begin{enumerate}
  ...
  \begin{description}
    ...
    \item[Distant]
    spells throw your magic far into the distance.
    ...
    \begin{itemize}
      \item
      A \textit{Divergent Wax Fire} spell could add \textit{Water}, and encourage a nearby stream to suddenly overflow, while a lantern consumes all its fuel in a fiery burst.
    \end{itemize}
    \item[Duplicated]
...
Spells targeting many people inflict the same fate on each of them, so if they split up, they each receive encounters.
\iftoggle{judgement}{%
...
\paragraph{Detailed}
spells allow the caster to specify what kind of encounter they might have -- a standard encounter with creatures, or with a trader.
The encounter must fit wherever the target happens to be at the time, or nothing happens.
